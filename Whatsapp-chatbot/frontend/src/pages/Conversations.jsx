...
import { getConversations, getConversation, replyToConversation, updateStatus, updateTags } from '../services/api';
  ...
  useEffect(() => {
    ...
    socket.on('new_message', ({ phone, name, message }) => {
      ...
      if (Notification.permission === 'granted') {
        new Notification(`📱 ${name || phone}`, { body: message.body });
      }
      loadList();
          ...
          {list.map(c => (
            <div key={c.phone} onClick={() => selectConv(c.phone)}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                ...
                <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 10, background: STATUS_COLORS[c.status], color: '#fff' }}>
              ...
              {c.tags?.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                  {c.tags.map(t => (
                    <span key={t} style={{ fontSize: 10, padding: '1px 6px', borderRadius: 8, background: '#e0f2fe', color: '#0369a1' }}>{t}</span>
                  ))}
                </div>
              )}
              {c.budgetFlow?.step === 4 && (
                <div style={{ marginTop: 3, fontSize: 11, color: '#7c3aed' }}>📋 Presupuesto recibido</div>
              )}
            </div>
          ))}
        </div>
          ...
          <>
            <div style={{ padding: '12px 20px', borderBottom: '1px solid #eee' }} onClick={() => setShowTagMenu(false)}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                {(chat.tags || []).map(t => (
                  <span key={t} onClick={(e) => { e.stopPropagation(); toggleTag(t); }} title="Click para quitar"
                    ...
                    <div style={{ position: 'absolute', top: 26, left: 0, background: '#fff', border: '1px solid #eee', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.12)', zIndex: 10, minWidth: 190 }}>
                      {ALL_TAGS.map(t => (
                        <div key={t} onClick={(e) => { e.stopPropagation(); toggleTag(t); setShowTagMenu(false); }}
                          style={{ padding: '8px 14px', cursor: 'pointer', fontSize: 13, background: (chat.tags || []).includes(t) ? '#f0fdf4' : '#fff',
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 10, background: '#ece5dd' }}
              ...
              {chat.messages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.from === 'customer' ? 'flex-start' : 'flex-end' }}>
                  <div style={{
                    maxWidth: '70%', padding: '8px 14px', borderRadius: 10, fontSize: 14, whiteSpace: 'pre-wrap',
                    background: m.from === 'customer' ? '#fff' : m.from === 'bot' ? '#dcf8c6' : '#d1e8ff',
                    boxShadow: '0 1px 2px rgba(0,0,0,0.1)'
                  }}>
                    {m.from !== 'customer' && (
                      <div style={{ fontSize: 10, color: '#888', marginBottom: 2 }}>{m.from === 'bot' ? '🤖 Bot' : '👤 Agente'}</div>
                    )}
                    {m.body}
