  ...
  try {
    ...
    if (q) {
      ...
      const searchClauses = [
        ...
        { "items.detalle": rx },
        { "items.descripcion": rx },
        { "items.nombre": rx },
        { "items.producto": rx },
        { "items.tipo": rx },
        { "items.busqueda": rx },
        { "items.data.descripcion": rx },
        { "items.data.nombre": rx },
        { "items.data.producto": rx },
        { "items.data.tipo": rx },
        { "items.data.codigo": rx },
      ];
    ...
    const [items, total] = await Promise.all([
      NotaPedido.find(filter).select(listSelect).sort({ _id: -1 }).skip(skip).limit(l).lean(),
